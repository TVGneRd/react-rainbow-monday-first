...
var _getHeaderRowFromWorkbook = _interopRequireDefault(
    require('./helpers/getHeaderRowFromWorkbook'),
);

var _getDataToImport = _interopRequireDefault(require('./helpers/getDataToImport'));

var _isStepThreeNextButtonDisabled = _interopRequireDefault(
    require('./helpers/isStepThreeNextButtonDisabled'),
);
...
function ImportRecordsFlow(props) {
    ...
    var goNextStep = function goNextStep() {
        ...
        if (currentStepIndex === 3) {
            onComplete(
                _objectSpread(
                    _objectSpread({}, mappedRecords.current),
                    {},
                    {
                        data: validRecords,
                    },
    ...
    var assignField = function assignField(databaseFieldToAssign, fileFieldsToAssign) {
        setFieldsMap(
            _objectSpread(
                _objectSpread({}, fieldsMap),
                {},
                (0, _defineProperty2['default'])(
                    {},
                    databaseFieldToAssign,
                    fileFieldsToAssign.join(','),
                ),
            ),
        );
    };
